...
var infoElt, xpiLinksCache;
this.__defineGetter__("doc", function() {
  return infoElt ? infoElt.ownerDocument : null;
})
...
exports.gFormatter = {
  ...
  formatInfo: function(aInfoElt, aXpiLinksCache, aHash, aNeedXPILinks) {
    gHash = aHash;
    xpiLinksCache = aXpiLinksCache;
    infoElt = aInfoElt;
    if(!infoElt) return "";
    ...
    var uri = this._selectedTemplateURI;
    if(uri == "") {
      var errMsg = ILHelpers.stringBundle.GetStringFromName("select_template");
    ...
    try {
      ...
      return ILHelpers.stringBundle.GetStringFromName("internal_error");
    }
  ...
  // used internally in format.js, but also in the options window
  loadTemplate: function(uri) {
    try {
      ...
      //XXX LOG(e);
      var errMsg = ILHelpers.stringBundle
...
function walkTree()
{
  ...
  var disabledText = ILHelpers.stringBundle.GetStringFromName("item_disabled");
  var selectedText = ILHelpers.stringBundle.GetStringFromName("item_selected");
